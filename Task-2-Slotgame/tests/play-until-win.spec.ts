...
// Emulation using iPhone 13 device, but keeping Chromium as the engine
test.use({ ...devices['iPhone 13'], defaultBrowserType: 'chromium' });

const GAME_URL = 'https://zig.services/games/gazicashkiosk';
const MAX_SPINS = 40; // observed some win chance per spin, so kept 40 comfortably covering a bad streak but can be increased if needed

function findGameFrame(page: Page) {
  // The game boots into a nested iframe chain (modal -> outer.html -> inner.html).
  // inner.html is where the actual game UI lives (canvas).
...
test('play the game until a win occurs and check the win display matches the API', async ({ page }, testInfo) => {
  ...
  // The click occasionally lands before the page has finished loading and does nothing, so give it a couple of retries until the game frame shows up.
  ...
  // The game fades in from a loading splash screen - give that a moment to finish so the click below actually lands on the real spin button.
  ...
  for (let spin = 1; spin <= MAX_SPINS && !win; spin++) {
    ...
    // The round isn't done until the ticket settles and the balance is re-fetched - that's also what re-enables the play button for the next spin.
    await settled;
